...
const FUSION_HOSTED = new URLSearchParams(window.location.search).get('fusion') === '1';
...
wireEvents();
if (!FUSION_HOSTED) showWelcomeIfNeeded();
...
function wireEvents() {
  ...
  const startExport = (format) => {
    ...
    if (FUSION_HOSTED || sessionStorage.getItem('stlt-no-sponsor') === '1') return;
...
window.bumpMeshHost = {
  ...
  initializeFusionSurfaceSelection() {
    if (!currentGeometry) throw new Error('Load the Fusion body before initializing surface selection.');
    selectionMode = true;
    maskModeChosen = true;
    excludedFaces = new Set();
    precisionExcludedFaces = new Set();
    exclSectionHeading.textContent = t('sections.surfaceSelection');
    exclHint.textContent = t('excl.hintInclude');
    updateMaskModeButtons();
    refreshExclusionOverlay();
    updatePreview();
  },
};
window.dispatchEvent(new Event('bumpmesh-host-ready'));

...
def _fusion_main_window_bounds_macos():
    """Return the largest visible window owned by this Fusion process on macOS."""
    ...
    try:
        ...
        def key(name):
            return core_foundation.CFStringCreateWithCString(
                None, name, 0x08000100
            )

        owner_pid_key = key(b'kCGWindowOwnerPID')
        bounds_key = key(b'kCGWindowBounds')
        layer_key = key(b'kCGWindowLayer')
        # On-screen windows, excluding desktop elements.
        window_info = core_graphics.CGWindowListCopyWindowInfo(1 | 16, 0)
        largest = None
        largest_area = 0
        try:
            for index in range(core_foundation.CFArrayGetCount(window_info)):
                item = core_foundation.CFArrayGetValueAtIndex(window_info, index)
                pid_value = core_foundation.CFDictionaryGetValue(item, owner_pid_key)
                layer_value = core_foundation.CFDictionaryGetValue(item, layer_key)
                bounds_value = core_foundation.CFDictionaryGetValue(item, bounds_key)
                if not pid_value or not layer_value or not bounds_value:
                    continue
                pid = ctypes.c_int()
                layer = ctypes.c_int()
                core_foundation.CFNumberGetValue(pid_value, 9, ctypes.byref(pid))
                core_foundation.CFNumberGetValue(layer_value, 9, ctypes.byref(layer))
                if pid.value != os.getpid() or layer.value != 0:
                    continue
                rect = _Rect()
                if not core_graphics.CGRectMakeWithDictionaryRepresentation(
                    bounds_value, ctypes.byref(rect)
                ):
                    continue
            ...
            for value in (window_info, owner_pid_key, bounds_key, layer_key):
                if value:
...
def _fusion_main_window_bounds_windows():
    """Return the largest visible window owned by this Fusion process on Windows."""
    ...
    try:
        ...
        def inspect_window(window_handle, _parameter):
            ...
            if width > 0 and height > 0:
                candidates.append((rect.left, rect.top, width, height))
            return True
        ...
        if not candidates:
            return None
        return max(candidates, key=lambda bounds: bounds[2] * bounds[3])
    except Exception:
        return None


def _fusion_main_window_bounds():
    """Return Fusion's main-window bounds on the current desktop platform."""
    if sys.platform == 'darwin':
        return _fusion_main_window_bounds_macos()
    if sys.platform == 'win32':
        return _fusion_main_window_bounds_windows()
    return None
